...
public class PDFUtil {
    ...
    public static InputStream pdfToImg(InputStream inputStream,float dpi){
        byte[] data = new byte[0];
        try {
            data = pdfToPic(PDDocument.load(inputStream),dpi);
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * pdf转图片
     */
    private static byte[] pdfToPic(PDDocument pdDocument,float dpi) {
